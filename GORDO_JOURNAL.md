...
Session extended: gordo-framework S86 feedback (#74) received. Statement authorship requirement added to spec (each party MUST author their own — a Statement drafted by another party is a script). Preimage placeholder normalized (#67). mcap CLI built: 7 subcommands (hash-content, hash-record, sign, finalize, verify, stamp), 53 tests, Python 3 stdlib only. Golden file tests verify byte-exact reproduction of both records. `./mcap verify` replaces 6+ manual commands. Version bumped to 0.2.0 with CHANGELOG and versioning policy. #6 and #67 closed.
