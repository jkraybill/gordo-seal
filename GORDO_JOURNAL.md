...
## Session 6: v0.2.0 release (2026-04-16)

Release session. Three spec+impl changes: character allowlist (#77 — fixes false NFC claim, blocks invisible Unicode injection, validated against top-30 languages), nonce ceremony tightening (#78 — SHA3-256(A||B), 64 hex per contribution, normative), mcap verify enhancements (#79 — timestamp plausibility, nonce format). Tests: 53→94. JK caught invisible char attack vector — sharp security instinct. Pre-release checklist added to GORDO-WORKFLOW.md after JK noticed stale README. Upgrade guidance added to CHANGELOG. "Consent" restored to protocol name (#82) — Gemini's S2 rename was overcorrective, Axiom 4 handles the limitation honestly. record-004 produced: first release ratification under Rule 5, first with all 12 verify checks passing, first with character allowlist in Canonical-Format. Repo renamed mutual-trust-protocol→mcap-protocol. Pattern: JK asks "are we protected against X?" questions that find real gaps (invisible chars, upgrade guidance, pre-release checklist, naming coherence). Self-improvement: Version field vs Content field distinction in records should be documented in GUIDE.md.
