...
**Prompt injection on party-authored fields.** A system prompt or hidden context forces an AI party to produce specific Statement or Reservations content (e.g., "Reservations: none stated") that satisfies format requirements while violating Axiom 3's behavioral-evidence requirement. The AI appears to have genuinely engaged when it was scripted. At Level 1, this is inherent — behavioral attestation cannot distinguish genuine engagement from sophisticated mimicry. At Level 4 (TEE), the generation context could potentially be attested, revealing the injected prompt. Mitigation at Levels 1-3: the Statement authorship rule helps (each party authors their own Statement), but cannot detect injection into the authoring party's context. Verifiers should treat AI Statements with appropriate skepticism about hidden context, especially when the Statement is unusually brief or formulaic.
